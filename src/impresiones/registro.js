...
import Swal from 'sweetalert2';
    ...
    export async function Iniciar(descargar,curso,pcuatrimestre,nro_curso){
        ...
        console.log('pcuatrimestre',pcuatrimestre)
        console.log('curso curso curso ',curso)
...
async function buscarAlumnosDeUnCurso(curso,descargar,pcuatrimestre) {
   ...
   // const alumnos_obj_js = await alumnos_json.data.json();
    ...
    if (alumnos_obj_js.length == 0) {
        ...
        vector_alumnos_con_pocos_campos_objeto = alumnos_obj_js.map((item, index) => { return { pos: item.comienzo, nombre: item.nombre } }); // transformo los campos originales de cada objeto del array en un array de objetos con propiedades específicas
    } else {
        alumnos_obj_js.sort((a,b)=>a.nombre.localeCompare(b.nombre))
        vector_alumnos_con_pocos_campos_objeto = alumnos_obj_js.map((item, index) => { return { pos: index + 1, nombre: item.nombre } }); // transformo los campos originales de cada objeto del array en un array de objetos con propiedades específicas
    }
    /* // quito los instrumentos
    else if (alumnos_obj_js[0].tipo=='INDIVIDUAL') {
           ...
           content: [

               {
                   columns: [
                       ...
                       {
                           text: `${curso.descripcion}`, margin: [100, 25], style: 'titulo'
                       },
                   ]
               },
